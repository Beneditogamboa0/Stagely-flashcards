(() => {
  ...
  */
  ...
  insertContent(
    ...
    `R O problema central é a inexistência de uma plataforma centralizada para o acompanhamento e gestão dos estágios curriculares no IPDDF. O processo dependia de métodos manuais ou parcialmente informatizados, o que gerava dificuldades de comunicação entre os intervenientes, atrasos na avaliação, perda de documentos e um acompanhamento pouco eficiente dos estagiários.`,
  );

  insertContent(
    "2. Qual é a diferença entre problema e problemática?",
    `R: A diferença entre problemática e problema é que a problemática é o contexto geral da situaçãoque gera o problema, enquanto o problema é a questão central específica que a pesquisa pretende responder.`,
  );

  insertContent(
    "3. Qual é a questão científica do vosso trabalho?",
    `R: De que maneira a criação de uma aplicação web pode contribuir para a modernização e melhoria do acompanhamento e gestão de estágios no IPDDF?`,
  );

  insertContent(
    "4. O que é a questão científica?",
    `R: É a questão inicial, marca o referencial que lança o pesquisador ao seu trabalho de pesquisa.`,
  );

  insertContent(
    "5. O que é hipóteses?",
    `R: Hipótese é uma resposta que se sugere para uma questão proposta como problema de investigação.`,
  );

  insertContent(
    "6. Quias são as hipóteses do vosso trabalho?",
    `R: H1: A implementação de uma aplicação para gestão de estágios melhorará significativamente a eficiência dos processos administrativos relacionados ao controlo e acompanhamento dos estágios curriculares no IPDDF. H2: A utilização de uma aplicação reduzirá o tempo gasto em tarefas administrativas relacionadas à gestão de estágios, tornando o processo mais efficiente para os intervenientes. H3: A centralização das informações sobre o estágio numa única aplicação facilitará a comunicação entre as partes envolvidas e minimizará erros e perdas de dados.`,
  );

  insertContent(
    "7. Qual é a importância deste projecto?",
    `R: Este projecto é importante porque melhora todo o processo de acompanhamento e gestão de estágios. Com a aplicação, as informações ficam centralizadas num único sistema, reduzindo perdas de dados, melhorando a organização e facilitando a comunicação entre os intervenientes.`,
  );
  ...
  insertContent(
    ...
    `R: Objectivo geral do nosso tema é desenvolver uma aplicação web para facilitar o acompanhamento e a gestão dos estágios curriculares no IPDDF, visando melhorar a eficiência, organização e qualidade do processo de supervisão.`,
  );

  insertContent(
    "10. Quais são os objectivos específicos do vosso trabalho?",
    `R: Os objectivos específicos são: Rever a literatura existente relacionada com o tema em estudo, de forma a fundamentar teoricamente o desenvolvimento da aplicação; Avaliar o actual processo de acompanhamento e gestão de estágios no IPDDF, identificando as principais limitações e necessidades dos intervenientes; Analisar os requisitos funcionais e não funcionais necessários para o desenvolvimento da aplicação de acompanhamento de estágios curriculares; Testar a funcionalidade e a usabilidade da aplicação desenvolvida, no sentido de melhorar a eficiência, a organização e a qualidade no processo de supervisão de estágios.`,
  );

  insertContent(
    "11. Qual é a diferença entre objectivo geral e objectivos específicos?",
    `R: O objectivo geral representa a meta principal do projecto, ou seja, aquilo que se pretende alcançar de forma ampla. Já os objectivos específicos são etapas mais detalhadas que ajudam a atingir o objectivo geral.`,
  );

  insertContent(
    "12. Define estágio curricular.",
    `R: Estágio curricular é um processo de aprendizagem prática que visa complementar a formação académica, promovendo a integração entre teoria e prática.`,
  );

  insertContent(
    "13. Qual é a diferença entre acompanhamento e gestão de estágios?",
    `R: Acompanhamento é o processo de supervisão contínua do desempenho do estagiário. Gestão envolve planejar, organizar, dirigir e controlar todo o processo de estágio.`,
  );

  insertContent(
    "14. Define aplicação web.",
    `R: Aplicação web é um programa que reside num servidor e é acedido por meio de um navegador, permitindo que os utilizadores interajam com conteúdos dinâmicos e base de dados através da rede.`,
  );

  insertContent(
    "15. Define sistema de informação.",
    `R: Sistema de informação constituem uma área multidisciplinar que combina tecnologia, organizações e pessoas para recolher, filtrar, processar, criar e distribuir dados.`,
  );

  insertContent(
    "16. Quais são os componentes de um Sistema de Informação?",
    `R: Os componentes de um sistema de informação são: hardware, software, dados, redes e pessoas.`,
  );

  insertContent(
    "17. O que é software?",
    `R: Software é o conjunto de instruções e programas que permitem ao computador executar tarefas. É a parte lógica do sistema computacional.`,
  );

  insertContent(
    "18. Quais são as categorias de software?",
    `R: As categorias de software são: Software de sistema: controla e gere os recursos do hardware. Exemplos: Windows, Linux, Android. Software de aplicação: atende às necessidades específicas dos utilizadores. Exemplos: Microsoft Word, navegadores de internet. Software de programação: utilizado por desenvolvedores para criar outros programas. Exemplos: Python, Java, PHP, Visual Studio Code.`,
  );

  insertContent(
    "19. que é Engenharia de Software?",
    `R: Engenharia Software é a disciplina que aplica princípios de engenharia ao desenvolvimento de software, garantindo qualidade, organização e eficiência durante todo o ciclo de vida do sistema.`,
  );

  insertContent(
    "20. O que é ciclo de vida?",
    `R: Ciclo de vida é o conjunto de etapas que orientam o desenvolvimento do sistema, desde o planeamento até à manutenção.`,
  );

  insertContent(
    "21. Quais são as fases do ciclo de vida?",
    `R: As fases do ciclo de vida são: ✓ Planeamento Análise ✓ Projecto ✓ Implementação ✓ Testes ✓ Implantação ✓ Manutenção`,
  );

  insertContent(
    "22. Porque é importante adoptar boas práticas de desenvolvimento?",
    `R: Adopção de boas práticas é essencial para o sucesso de projectos de software, pois reduz a probabilidade de falhas, facilita a manutenção e aumenta a satisfação dos utilizadores`,
  );

  insertContent(
    "Por que escolheram HTML, CSS, JavaScript, PHP e MySQL?",
    `R: Escolhemos essas tecnologias porque: São open source (baixo custo); São amplamente utilizadas no desenvolvimento web; ✓ São compatíveis entre si; ✓ São adequadas ao contexto tecnológico da instituição.`,
  );
  ...
  insertContent(
    ...
    `R: É possível garantir a integridade dos dados através do: ✓ Uso de chaves primárias e estrangeiras ✓ Validações no backend ✓ Restrições no banco de dados ✓ Controle de permissões`,
  );

  insertContent(
    "44. Como protegeram o sistema contra SQL Injection?",
    `R: Protegemos o sistema contra o SQL Injection através da: ✓ Validação e sanitização de entradas ✓ Uso de prepared statements ✓ Restrição de permissões no banco de dados`,
  );

  insertContent(
    "45. Qual é a arquitetura do sistema desenvolvido?",
    `R: O sistema segue o modelo cliente-servidor, onde: O frontend foi desenvolvido com HTML, CSS e JavaScript. O backend foi desenvolvido em PHP. A configuração e persistência dos dados é feita através de banco de dados relacional utilizando SQL. A comunicação ocorre através de requisições HTTP entre navegador e servidor.`,
  );

  insertContent(
    "46. Quais foram os principais benefícios da aplicação?",
    `R: Os principais beneficios da aplicação foram: ✓ Organização das informações ✓ Melhor comunicação ✓ Centralização dos dados ✓ Maior controlo dos estágios`,
  );

  insertContent(
    "47. Quais foram as limitações do projeto?",
    `R: A principal limitação foi a necessidade futura de integração com sistemas internos da instituição para automatizar importação de dados.`,
  );

  insertContent(
    "48. O sistema está pronto para uso real?",
    `R: Ele está funcional e testado em ambiente académico, mas para implementação oficial seriam necessários testes adicionais, ajustes de segurança e validação institucional.`,
  );

  insertContent(
    "49. Vocês atingiram os objetivos propostos?",
    `R: Sim. Conseguimos: ✓ Avaliar o processo actual; ✓ Desenvolver a aplicação; ✓ Testar funcionalidades e usabilidade; ✓ Identificar melhorias necessárias.`,
  );

  insertContent(
    "50. O que é um banco de dados?",
    `R: É um conjunto organizado de dados relacionados, armazenados de forma estruturada para facilitar consulta, inserção e actualização.`,
  );

  insertContent(
    "51. Como funciona a comunicação entre frontend e backend?",
    `R: O frontend envia dados através de formulários ou requisições HTTP (GET/POST). O PHP processa esses dados no servidor, interage com o banco de dados e retorna a resposta ao navegador.`,
  );

  insertContent(
    "52. O que é chave primária?",
    `R: É o campo que identifica unicamente cada registo numa tabela.`,
  );

  insertContent(
    "53. O que é chave estrangeira?",
    `R: É um campo que cria ligação entre duas tabelas, garantindo integridade referencial.`,
  );

  insertContent(
    "54. Como garantiram a segurança do sistema?",
    `R: Garantimos a segurança do sistema através de: ✓ Login obrigatório ✓ Validação de campos ✓ Controle de acesso por tipo de usuário ✓ Organização adequada da base de dados ✓ Criptografia das senhas ✓ Utilizamos boas práticas para evitar vulnerabilidades como SQL Injection.`,
  );

  insertContent(
    "55. que é criptografia?",
    `R: É o processo de transformar informações em formato protegido para evitar acessos não autorizados.`,
  );
